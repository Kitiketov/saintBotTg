...
async def join_room(msg: Message, state: FSMContext):
    ...
    if msg.text == "🚫Отмена":
        ...
        return
    room_status = await db.connect2room(name,msg.chat.id)
...
async def create_room(msg: Message, state: FSMContext):
    ...
    if msg.text == "🚫Отмена":
        ...
        await msg.answer("Меню", reply_markup=keyboards.choice_kb)
        return
    id = await db.create_room(name,msg.chat.id)
    if not id:
    ...
    await msg.answer(f"Комната:  {name}:{id} созданна \nЧтобы другие могли в неё войти скажите им её название c id",reply_markup=kb)
...
async def cancel(call: CallbackQuery, callback_data: CallbackFactory, state: FSMContext):
    await db.leave_room(callback_data.room_iden,call.message.chat.id)
...
async def get_my_admin_rooms(call: CallbackQuery, callback_data: CallbackFactory, state: FSMContext):
    rooms = await db.get_my_rooms(call.message.chat.id, callback_data.asAdmin)
...
async def show_room_as_admin(call: CallbackQuery, callback_data: CallbackFactory, state: FSMContext):
...
async def show_room_as_admin(call: CallbackQuery, callback_data: CallbackFactory, state: FSMContext):
    await db.delete_room(callback_data.room_iden,call.message.chat.id)
...
async def show_room_as_admin(call: CallbackQuery, callback_data: CallbackFactory, state: FSMContext):
    ...
    #TODO Авто  рассылка, проверка начала, проверка на наличие участников больше 1
